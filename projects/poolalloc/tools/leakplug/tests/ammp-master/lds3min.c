//
//  take two coupled  torsions that share an atom
//  ca cb and ca c are the central idea, but it could be others.
//
...
int lds3min( op,vfs,ffs, nfs, at1,at2,at3, temper, nstep)
	FILE *op;
...
/* now do the second possible torsion  which is a2 - (instance of at3) */
		get_bond( a2,bonded, 10, &in_bond);
		/* get a1 and a3  a1 != (instance of) at3*/
		a1 = NULL;
		for( i=0; i< in_bond; i++)
			if( math_match_atom( at3,bonded[i]) == 0)
			{a1 = bonded[i]; break;}
		if( a1 == NULL) goto RE_CHOOSE;
		a3 = NULL;
		for( i=0; i< in_bond; i++)
...
/* set_torsion uses (or should use a) delta_value */
	...
	for( i=0; i< 20; i++)
		dV += dt*(2.*randf()-1.);
	dV /= 20;
	if( dV > 5./360*2.*3.14159265) dV = 5./360*2.*3.14159265;
	if( dV < -5./360*2.*3.14159265) dV = -5./360*2.*3.14159265;
	set_torsion( a1,a2,a3,a4, dV );
	fprintf(op,"%f %f \n",dV,get_torsion_value( a1,a2,a3,a4 ));
RE_CHOOSE: ;
