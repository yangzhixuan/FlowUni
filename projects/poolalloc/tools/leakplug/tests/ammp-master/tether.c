...
float fk ,x,y,z;
{
    ...
    if( (ap1 == NULL)  )
    {
        ...
        aaerror( line );
      /*  return 0; it is OK to skip a tether if it is not defined*/
        return 1;
    }
...
/* skip xxx.[[1|9]h] atoms (hydrogens) */
int no_h_tether( fk)
float fk ;
{
    ATOM *ap1 ;
...
float tether_error( ATOM *ap)
{
   	TETHER *bp;
        float xt,yt,zt;
        ATOM *a1;
    	bp = tether_first;
    	if( bp == NULL ) return 10.;
    	while(1)
    	{
	a1 = bp->atom1;
	if( a1 == ap)
