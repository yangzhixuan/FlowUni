...
FILE *op;
{
    ...
    int v_dssp_hbond(), gsdg_dssp_hbond();
    int f_bond(),f_angle(),f_noel();
    int kohonen_minimizer();
    int (*lp[3])(),(*lf[3])(),ilocal;
    int use_noel, use_bond,use_angle, use_hybrid, use_nonbon, use_dssp;
    void rand3(),gsdg();
#ifdef WINDOZ
    ...
    for( i=0; i< nused; i++)
    {
        ...
        if( pot[i] == v_dssp_hbond) use_dssp = (1==1);
    }
    ilocal = 0;
if( use_bond) {lf[ilocal] = f_bond; lp[ilocal++] = v_bond;}
    ...
    for( icycle=0; icycle< ncycles; icycle ++)
    {
        ...
        for( iatom =0; iatom < numatom; iatom ++)
        {
            ...
            if( use_dssp) gsdg_dssp_hbond(ap);
  ...
  /*                              bp->vx = 16.; */
				if( icycle == 0)
					...
					bp->vx = 25 - 21.*(float)icycle/(ncycles-1);
                            }
                            /*			} */
                        }
                    }
