...
/*  trying just the closest so this is commented out
* for noisy data
        ...
        if( r < tp->r[0] || r > tp->r[ tp->inme-1])
        {
            ...
            */
            ...
            /*
            	...
            	*/
/* original differential is bogus?
...
*/
	dr = rclose[0]-rclose[1];
	dvl = -vclose[0]*rclose[2]/(rclose[0]-rclose[2])*(1.-1./(dr*dr));
	dr = rclose[0]-rclose[2];
	dvl += -vclose[0]*rclose[1]/(rclose[0]-rclose[1])*(1.-1./(dr*dr));
	dr = rclose[1]-rclose[0];
	dvl += -vclose[1]*rclose[2]/(rclose[1]-rclose[2])*(1.-1./(dr*dr));
	dr = rclose[1]-rclose[2];
	dvl += -vclose[1]*rclose[0]/(rclose[1]-rclose[0])*(1.-1./(dr*dr));
	dr = rclose[2]-rclose[0];
	dvl += -vclose[2]*rclose[1]/(rclose[2]-rclose[1])*(1.-1./(dr*dr));
	dr = rclose[2]-rclose[1];
	dvl += -vclose[2]*rclose[0]/(rclose[2]-rclose[0])*(1.-1./(dr*dr));
            vector[1] = 0.;
