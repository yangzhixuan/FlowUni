...
float grid,guard ; /* the grid (i.e., .5A) and guard radii */
{
    ...
    char line[81];  // for emulating FORTRAN IO AAAAAARGGGHHH!
    void dscf_map_setup();
...
*/
    ...
    fprintf( where, "%s\n",line);
    sprintf(line,"%s","REMARKS AMMP xplor map from dscf");
    for(i=0; i<80; i++) if( line[i] < ' ' ) line[i] = ' ';
    fprintf( where, "%s\n",line);
    for ( i=0; i< 80; i++) line[i] = ' ' ; line[80] = '\0';
    if(type == 0) sprintf(line,"%s","REMARKS QM edmap");
    if(type == 1) sprintf(line,"%s","REMARKS QM field map");
    if(type == 2) sprintf(line,"%s","REMARKS Empirical charge field map");
    for(i=0; i<80; i++) if( line[i] < ' ' ) line[i] = ' ';
    fprintf( where, "%s\n",line);
