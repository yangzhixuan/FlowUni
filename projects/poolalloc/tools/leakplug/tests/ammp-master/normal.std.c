...
/* frequency_cutoff could be zero  - if so make it very big so everything gets written */
    ...
    for( iatom = 0; iatom < numatm; iatom++)
    {
        ...
        bp->dz = 0.;
	printf("atom %d done with matrix \n", bp->serial); fflush(stdout);
        bp = bp->next;
    }
   ...
   // if( jacobi( norm,vect,numatm*3, 100*numatm*numatm, 1.e-7) != 0)
   // if( jacobi( norm,vect,numatm*3, 100*numatm*numatm, 1.e-5) != 0)
    if( jacobi( norm,vect,numatm*3, 100*numatm*numatm, 1.e-3) != 0)
    {/* error condition */
