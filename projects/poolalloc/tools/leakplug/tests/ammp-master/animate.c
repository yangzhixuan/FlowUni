...
* just recenter the velocities
*/
int v_constrain_center( V, lambda  )
float *V, lambda;
{
int f_constrain_center( );
return f_constrain_center( lambda );
}
int f_constrain_center( l)
float l;
{
	int i, numatm, a_number();
	ATOM *ap, *a_next();
	float cx,cy,cz, ke, ke2;
	...
	for( i=0; i< numatm; i++)
	{
		...
		ap->vy -= cy;
		ap->vz -= cz;
		ke2 += ap->mass*( ap->vx*ap->vx + ap->vy*ap->vy + ap->vz*ap->vz);
	}
	ke = ke/ke2;
	for( i=0; i< numatm; i++)
	{
